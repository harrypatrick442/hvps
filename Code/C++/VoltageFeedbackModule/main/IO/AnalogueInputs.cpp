...
void AnalogueInputs::initialize()
{
    ...
    // start with default channel (voltage divider)
    selectVoltageDividerInputAsChannel();
...
void AnalogueInputs::selectVoltageDividerInputAsChannel()
{
...
MonitorVoltageThresholdHandle* AnalogueInputs::monitorVoltageThresholdWithNewPriorityTask(double initialVoltage) {
	MonitorVoltageThresholdHandle* handle = new MonitorVoltageThresholdHandle(
		initialVoltage, _reverseLookup);
...
void AnalogueInputs::_monitorVoltageThreshold(void* pointer) {
	MonitorVoltageThresholdHandle* handle = (MonitorVoltageThresholdHandle*)pointer;
	...
	while(!handle->exit){
		...
		if(value>handle->rawThreshold){
			if((!set)||isOn){
				isOn = false;
				Outputs::setThresholdReachedFiberOpticOnOff(false);
			}
		}
		else{
			if((!set)||(!isOn)){
				isOn = true;
				Outputs::setThresholdReachedFiberOpticOnOff(true);
			}
		}
		set = true;
	}
