...
export default class Select{
    constructor (props) {
		...
		this._readOnlyElement.classList.add('readonly');
	...
	setSelectedValue(value, skipUpdateToModel) {
		...
		for (var i = 0; i < this._select.options.length; i++) {
			...
			if (option.value === valueString) {
				...
				this._readOnlyElement.textContent = option.text;
				foundOption = true;
				break;
			}
		}
		...
		if(!foundOption){
			...
			if(this._select.options[0])
				this._readOnlyElement.textContent=this._select.options[0].text;
		}
		...
		/*if(!this._loopBreakDefault.trigger()){
			if(!skipUpdateToModel)
				this._updateToModel(this._select.options[0]);
		}*/
		this.dispatchEvent({type:'change', selectedValue:value, selectedItem:this._selectedItem});
	}
	...
	_updateItems(items) {
		...
		this._mapValueStringToValue = {};
		items.forEach((item)=>{
			var value = this._itemValueName ? item[this._itemValueName] : (this._getterNameItemValue? item[this._getterNameItemValue]():item);// todo can make this more efficient when get time.
			...
			this._mapValueStringToValue[valueString]=value;
			this._select.appendChild(option);
	...
	_updateToModel(selectedOption){
		if (this._propertyNameSelectedItem) {
			this._selectedItem = this._mapValueToItems[selectedOption.value];
			...
			if(selectedOption.value==='undefined')
				...
				const value = this._mapValueStringToValue[selectedOption.value];
				this._selectedValue = this._valueConverter?this._valueConverter(selectedOption.value):value;
			}
